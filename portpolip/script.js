document.addEventListener('DOMContentLoaded', function () {
    ...
    const GITHUB_USERNAME = 'evilbigi74-bot';
    ...
    async function fetchRepoLanguages(repoName) {
        try {
            ...
            return languagesArray.sort((a, b) => parseFloat(b.percentage) - parseFloat(a.percentage)).slice(0, 5);
        } catch (error) {
            return null;
        }
    }
    ...
    function createProjectCard(repo) {
        ...
        // Fetch and display languages as progress bar
        const languagesBar = card.querySelector('.languages-bar');
        const languagesLabels = card.querySelector('.languages-labels');

        fetchRepoLanguages(repo.name).then(languages => {
            if (languages && languages.length > 0) {
                ...
                languages.forEach(lang => {
                    ...
                    segment.style.background = languageColors[lang.name] || 'hsl(var(--primary))';
                    segment.title = lang.name + ': ' + lang.percentage + '%';
                    languagesBar.appendChild(segment);
                });

                // Create labels below the bar
                languages.forEach(lang => {
                    ...
                    dot.style.background = languageColors[lang.name] || 'hsl(var(--primary))';
                    ...
                    languagesLabels.appendChild(label);
                });
            } else {
                languagesBar.style.display = 'none';
                languagesLabels.innerHTML = '<span class="language-label"><span class="language-name">No languages detected</span></span>';
            }
        });
